...
async def update_mcp_configuration(request: McpConfigUpdateRequest) -> McpConfigResponse:
    ...
    try:
        ...
        # _maybe_encrypt is a no-op unless MCP_ENCRYPT_SECRETS is set.
        config_data = dict(raw_other_keys)
        config_data["mcpServers"] = {name: _maybe_encrypt(server.model_dump()) for name, server in merged_servers.items()}
