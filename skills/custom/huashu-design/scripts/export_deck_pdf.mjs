 ...
 * - Background/graphics 1:1 fidelity (Playwright embedded Chromium rendering)
 * - No modification to HTML required
 * - Visual loss = 0 (PDF is what the browser prints out)
 *
 * trade-off：
 * - PDF text cannot be edited anymore (you need to change it back to HTML)
 *
 * Dependency: playwright pdf-lib
 *   npm install playwright pdf-lib
 *
 * Will be sorted by file name (01-xxx.html → 02-xxx.html → ...)
 */
