...
(async () => {
  ...
  if (hasReady) {
    // The second line of defense: proactively reset animation time to zero - dealing with HTML that does not strictly adhere to the starter tick template
    // (for example, using performance.now() on lastTick causes the font loading time to be included in the first frame dt)
    ...
    if (seekCorrected) {
      // Wait for two rAFs for seek to take effect and render the picture with t=0
