#!/usr/bin/env python3
"""
verify.py — Playwright package, used to verify the HTML produced by claude-design

Usage:
    python verify.py path/to/design.html                    # Basics: open + screenshot + capture console errors
    python verify.py design.html --viewports 1920x1080,375x667  # Multiple viewports
    python verify.py deck.html --slides 10                  # Slides are cut page by page (first 10)
    python verify.py design.html --output ./screenshots/   # Output directory
    python verify.py design.html --show                    # Non-headless, open the real browser
...
def verify_html(html_path, viewports=None, slides=0, output_dir=None, show=False, wait=2000):
    ...
    with sync_playwright() as p:
        ...
        for viewport in viewports:
            ...
            if show:
                print(" (Browser window remains open, press Enter to close...)")
                input()
    ...
    if console_errors:
        ...
        if len(console_errors) > 20:
            print(f" ... and {len(console_errors) - 20}")
    else:
        print("✅ Console clean")

    print(f"\n📸 Save the screenshot to: {output_dir}")
...
def main():
    ...
    parser.add_argument("--viewports", default="1440x900",
                        help="Comma separated list of viewports in format WxH (default 1440x900)")
    parser.add_argument("--slides", type=int, default=0,
                        help="Slideshow mode: intercept the first N pictures (requires HTML to support ArrowRight page turning)")
    parser.add_argument("--output", default=None,
                        help="Output directory (default screenshots/ of the directory where HTML is located)")
    parser.add_argument("--show", action="store_true",
                        help="Non-headless, open real browser window")
    parser.add_argument("--wait", type=int, default=2000,
                        help="Number of milliseconds to wait after opening the page (default 2000)")
