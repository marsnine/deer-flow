...
def pack_hwpx(input_dir: Path, output_path: Path, entry_order: list[str] | None = None) -> None:
    """Create HWPX archive with mimetype as first entry (ZIP_STORED).

    If entry_order is given, use that order (preserving original HWPX order).
    Otherwise, use sorted order.
    """
    ...
    if entry_order:
        all_files = [e for e in entry_order if e != "mimetype" and (input_dir / e).is_file()]
        # Add any new files not in original order
        existing = set(all_files) | {"mimetype"}
        ...
        all_files = sorted(
            ...
            for p in input_dir.rglob("*")
            if p.is_file() and p.relative_to(input_dir).as_posix() != "mimetype"
        )
...
def build(
    ...
    creator: str | None,
    output: Path,
    base_hwpx: Path | None = None,
) -> None:
    """Main build logic.

    If base_hwpx is given, use it as the base (preserving all auxiliary files
    like version.xml, settings.xml, etc. byte-for-byte). Otherwise fall back
    to the built-in base template directory.
    """

    with tempfile.TemporaryDirectory() as tmpdir:
        work = Path(tmpdir) / "build"

        original_entry_order: list[str] | None = None

        if base_hwpx:
            # Extract original HWPX as base — preserves all bytes exactly
            if not base_hwpx.is_file():
                raise SystemExit(f"Base HWPX not found: {base_hwpx}")
            from zipfile import ZipFile as ZF
            with ZF(base_hwpx, "r") as zf:
                original_entry_order = zf.namelist()
                zf.extractall(work)
        else:
        ...
        # 2. Apply template overlay (only when not using base_hwpx)
        if template and not base_hwpx:
        ...
        # 4. Update metadata (skip for base_hwpx to preserve original bytes)
        if not base_hwpx:
        ...
        # 6. Pack (preserve original entry order if available)
        pack_hwpx(work, output, entry_order=original_entry_order)
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-hwpx",
        type=Path,
        help="Use an existing HWPX file as base (preserves all auxiliary files byte-for-byte)",
    )
    parser.add_argument(
        "--output", "-o",
        type=Path,
